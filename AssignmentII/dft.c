...
int parallelDFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o,
                int N);
int opt_parallelDFT(int idft, double *xr, double *xi, double *Xr_o,
                    double *Xi_o, int N);
int main(int argc, char *argv[]) {
  ...
  // DFT
  int idft = 1;
  opt_parallelDFT(idft, xr, xi, Xr_o, Xi_o, N);
  // IDFT
  idft = -1;
  opt_parallelDFT(idft, Xr_o, Xi_o, xr_check, xi_check, N);
  // stop timer
...
int opt_parallelDFT(int idft, double *xr, double *xi, double *Xr_o,
  ...
  double idft_mult = idft == -1 ? inv_n : 1.0;

  double cos_table[N];
  double sin_table[N];
#pragma omp parallel for
  for (int n = 0; n < N; n++) {
    cos_table[n] = cos(n * PI2 * inv_n);
    sin_table[n] = idft * sin(n * PI2 * inv_n);
  }
...
#pragma omp parallel for
  for (int k = 0; k < N; k++) {
    for (int n = 0; n < N; n++) {
      int i = (n * k) % N;
      // Real part of X[k]
      Xr_o[k] += xr[n] * cos_table[i] + xi[n] * sin_table[i];
      // Imaginary part of X[k]
      Xi_o[k] += xi[n] * cos_table[i] - xr[n] * sin_table[i];
    }
    Xr_o[k] *= idft_mult;
    Xi_o[k] *= idft_mult;
  }
